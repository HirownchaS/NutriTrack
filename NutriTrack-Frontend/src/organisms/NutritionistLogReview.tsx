...
const NutritionistLogReview: React.FC = () => {
    ...
    const handleAudit = async (logId: string, status: 'approved' | 'rejected') => {
        try {
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 className="text-2xl font-black text-slate-800 tracking-tight">Food Log Audit</h2>
                    <p className="text-slate-500 text-sm font-medium mt-1">Review AI detections and verify nutritional accuracy.</p>
                </div>
                {users.length > 0 && (
                    <div className="flex items-center gap-4">
                        <div className="hidden md:flex gap-4 mr-4">
                            <div className="text-right">
                                <p className="text-[10px] font-black text-slate-400 uppercase">Total Kcal</p>
                                <p className="text-sm font-black text-emerald-600">{stats.totalCalories}</p>
                            </div>
                            <div className="text-right border-l border-slate-100 pl-4">
                                <p className="text-[10px] font-black text-slate-400 uppercase">Avg Kcal</p>
                                <p className="text-sm font-black text-blue-600">{stats.avgCalories}</p>
                            </div>
                        </div>
                        <select
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 gap-8">
                    {['breakfast', 'lunch', 'dinner', 'snacks'].map(mealType => {
                        const logs = logData.filter((log: any) => log.mealType === mealType);
                                ...
                                {logs.map((log: any) => (
                                    <Card key={log.id} className="p-6 border-emerald-50 flex flex-col md:flex-row gap-6 hover:shadow-md transition-shadow">
                                        ...
                                        <div className="flex-1 flex flex-col justify-between">
                                            <div>
                                                <div className="flex items-center gap-3 mb-2">
                                                    <h3 className="text-lg font-black text-slate-800">{log.foodName}</h3>
                                                    <span className="text-[10px] text-slate-400 font-bold bg-slate-50 px-2 py-0.5 rounded flex items-center gap-1">
                                                        <FiCalendar /> {log.date}
                                                    </span>
                                                </div>
                                                <div className="flex gap-3 text-xs">
                                                    <span className="text-emerald-600 font-black">{log.calories} kcal</span>
                                                    <span className="text-slate-400 font-bold">P: {log.protein}g</span>
                                                    <span className="text-slate-400 font-bold">C: {log.carbs}g</span>
                                                    <span className="text-slate-400 font-bold">F: {log.fats}g</span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="mt-4 flex gap-2">
                                                <Button
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        if (val) handleFeedbackChange(log.id, (auditFeedback[log.id] || '') + " [Adj: " + val + "]");
                                                    }}
                                                >
                                        ...
                                        <div className="flex flex-row md:flex-col justify-center gap-3 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6">
                                            {log.status === 'pending' ? (
                                                <>
                                                    <Button onClick={() => handleAudit(log.id, 'approved')} className="bg-emerald-500 hover:bg-emerald-600 text-white">
                                                        <FiCheckCircle className="mr-2" /> Approve
                                                    </Button>
                                                    <Button onClick={() => handleAudit(log.id, 'rejected')} variant="secondary" className="text-rose-500 border-rose-100 hover:bg-rose-50">
                                                        <FiXCircle className="mr-2" /> Reject
                                                    </Button>
                                                </>
                                            ) : (
                                                <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border font-black uppercase tracking-widest text-[10px] ${log.status === 'approved' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-600 border-rose-100'}`}>
                                                    {log.status === 'approved' ? <FiCheckCircle /> : <FiXCircle />}
                                                    {log.status}
                                                </div>
                                            )}
                                        </div>
