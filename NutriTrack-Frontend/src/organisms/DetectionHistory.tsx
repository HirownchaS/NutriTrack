...
import { getUserDetections, deleteDetection, DetectionHistoryItem } from '../services/firestore';
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col mt-8">
            <div className="px-6 py-5 border-b border-slate-100 flex flex-col md:flex-row items-center justify-between gap-4 bg-slate-50/50">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-lg font-bold text-slate-800">Detection History</h2>
                </div>
                ...
                <div className="flex bg-slate-100 p-1 rounded-xl">
                    <button
                        ...
                        className={`text-xs font-semibold px-4 py-2 rounded-lg transition-all ${
                            filter === 'today' ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                        }`}
                    >
                        Today
                    </button>
                    <button
                        ...
                        className={`text-xs font-semibold px-4 py-2 rounded-lg transition-all ${
                            filter === '7days' ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                        }`}
                    >
                        Last 7 Days
                    </button>
            ...
            <div className="p-6 text-sm text-slate-600">
                {isLoading ? (
                    ...
                    <div className="text-center py-10 opacity-60">
                        ...
                        <p className="font-medium text-slate-500">No detections found for this period.</p>
                        <p className="text-xs text-slate-400 mt-1">Upload a meal image above to get started.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {history.map((item) => (
                            <div key={item.id} className="bg-white border text-sm border-slate-200 rounded-2xl p-4 hover:shadow-md transition-shadow">
                                <div className="flex justify-between items-start mb-3 border-b border-slate-100 pb-3">
                                    <div>
                                        <p className="font-semibold text-slate-800 text-base flex items-center gap-2">
                                            {item.nutrition.totalCalories > 0 ? (
                                                <span className="text-emerald-600 font-bold">{item.nutrition.totalCalories} kcal</span>
                                            ) : (
                                                <span className="text-slate-400 font-medium">Unknown kcal</span>
                                            )}
                                        </p>
                                ...
                                <div className="space-y-2">
                                    {item.foods.length > 0 ? (
                                        ...
                                        <p className="text-xs text-center text-slate-400 italic py-2">No food items detected</p>
                                    )}
                                </div>
                            </div>
                        ))}
