...
const NutritionistUsers: React.FC = () => {
                    ...
                    const pending = snapshot.docs
                        .map((docSnap) => {
                            ...
                            return {
                                ...
                                status: rData.status,
                                date: rData.createdAt?.toDate ? rData.createdAt.toDate().toLocaleDateString() : 'Just now'
                            };
                        })
    ...
    const openChat = (userId: string, userName: string) => {
        navigate('/nutritionist-dashboard/chat', { state: { recipientId: userId, recipientName: userName } });
    };
    ...
    return (
        <div className="space-y-10 animate-fade-in">
            {toast && (
                <div className={`fixed top-6 right-6 z-50 px-5 py-3.5 rounded-2xl shadow-2xl text-sm font-bold animate-fade-in ${
                ...
                {pendingRequests.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {pendingRequests.map(req => (
                            <Card key={req.id} className="overflow-hidden border border-slate-100 group">
                                <div className="bg-orange-50 px-6 py-4 border-b border-orange-100/50 flex justify-between items-center">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h3 className="font-black text-slate-800 text-sm">{req.name}</h3>
                                ...
                                <div className="px-6 py-5 space-y-3">
                                    <div className="flex items-center gap-3 text-xs">
                                        ...
                                        <span className="font-bold text-slate-700">{req.email}</span>
                                    </div>
                                    <div className="flex items-center gap-3 text-xs">
                                        <FiTarget className="text-slate-400" />
                                        <span className="font-bold text-slate-700">{req.fitnessGoal} ({req.age || '?'}y, {req.weight || '?'}kg)</span>
                                    </div>
                                    <div className="text-[10px] bg-slate-100 p-2 rounded-lg text-slate-500 font-medium">
                                        <span className="font-black text-slate-700 uppercase mr-1">Condition:</span>
                                        {req.healthCondition}
                                    </div>
                                </div>
                                <div className="px-6 py-4 bg-slate-50/50 border-t border-slate-100 flex gap-3">
                                    <button onClick={() => handleRequest(req.id, 'rejected')} disabled={!!processingId} className="flex-1 py-2 rounded-xl text-xs font-bold text-rose-600 bg-rose-50 hover:bg-rose-100 border border-rose-100">Reject</button>
                                    <button onClick={() => handleRequest(req.id, 'accepted')} disabled={!!processingId} className="flex-1 py-2 rounded-xl text-xs font-bold text-white bg-emerald-500 hover:bg-emerald-600">Accept</button>
                                </div>
                            </Card>
                        ))}
                    </div>
            ...
            <section>
                ...
                {assignedUsers.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {assignedUsers.map(u => (
                            <Card key={u.id} className="p-6 hover:shadow-lg transition-all border-emerald-50/50">
                                <div className="flex items-center gap-4 mb-6">
                                    ...
                                    <div>
                                        ...
                                        <span className="text-[10px] font-black uppercase px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-600">Active User</span>
                                    </div>
                                </div>
                                <div className="space-y-2 mb-6 text-xs">
                                    <div className="flex justify-between"><span className="text-slate-400 font-bold">Goal</span><span className="text-slate-700 font-black">{u.fitnessGoal}</span></div>
                                    <div className="flex justify-between"><span className="text-slate-400 font-bold">Profile</span><span className="text-slate-700 font-black">{u.age}y | {u.weight}kg</span></div>
                                    <div className="flex justify-between"><span className="text-slate-400 font-bold">Health</span><span className="text-slate-700 font-black truncate max-w-[120px]">{u.healthCondition}</span></div>
                                </div>
                                <div className="flex gap-2">
                                    <button onClick={() => navigate(`/nutritionist-dashboard/progress?userId=${u.userId}`)} className="flex-1 py-2 px-2 bg-slate-50 text-slate-600 rounded-xl text-[10px] font-black border border-slate-100 hover:bg-emerald-50 hover:text-emerald-600 transition-all flex items-center justify-center gap-1">Profile <FiArrowRight /></button>
                                    <button onClick={() => openChat(u.userId, u.name)} className="flex-1 py-2 px-2 bg-emerald-500 text-white rounded-xl text-[10px] font-black shadow-md hover:bg-emerald-600 transition-all flex items-center justify-center gap-1">Chat <FiMessageCircle /></button>
                                </div>
                            </Card>
                        ))}
                    </div>
