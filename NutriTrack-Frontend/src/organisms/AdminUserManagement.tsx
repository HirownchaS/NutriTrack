...
import {
    FiSearch, FiEye, FiEdit2, FiTrash2, FiX, FiUser, FiMail,
    FiShield, FiActivity, FiTarget, FiUserCheck, FiAlertTriangle,
    FiCheck, FiChevronLeft, FiChevronRight, FiRefreshCw, FiSliders, FiUsers,
} from 'react-icons/fi';
...
const ToastContainer: React.FC<{ toasts: ToastItem[]; onRemove: (id: number) => void }> = ({ toasts, onRemove }) => (
    <div className="fixed top-5 right-5 z-[200] space-y-2 pointer-events-none">
        {toasts.map(t => (
            ...
            >
                {t.type === 'success' ? <FiCheck className="w-4 h-4 flex-shrink-0" /> : <FiAlertTriangle className="w-4 h-4 flex-shrink-0" />}
                <span>{t.message}</span>
                <button onClick={() => onRemove(t.id)} className="ml-2 opacity-70 hover:opacity-100 transition-opacity">
...
const RoleBadge: React.FC<{ role: string }> = ({ role }) => {
    ...
    return (
        <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider ${config[role] ?? config.user}`}>
            {role}
        </span>
    );
};
...
const AdminUserManagement: React.FC = () => {
        ...
        try {
            ...
            const raw: FirestoreUser[] = snap.docs.map(d => {
                ...
                return {
                    ...
                    email: data.email || '—',
                    role: data.role || 'user',
                    nutritionistId: data.nutritionistId || data.assignedNutritionistId || '',
            ...
            const idsToFetch = [...new Set(raw.map(u => u.nutritionistId).filter(Boolean))] as string[];
    ...
    const openModal = (user: FirestoreUser, type: ModalType) => {
        ...
        if (type === 'edit') {
            ...
            setEditCalorieGoal(user.dailyCalorieGoal !== undefined ? String(user.dailyCalorieGoal) : '');
        }
    };
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            addToast('Failed to delete user: ' + (err instanceof Error ? err.message : String(err)), 'error');
            setActionLoading(false);
        }
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const newNutriName = editNutritionistId
                ? (nutritionists.find(n => n.uid === editNutritionistId)?.name ?? 'Unknown')
                : 'Not Assigned';
            ...
            setUsers(prev => prev.map(u =>
                u.uid === selectedUser.uid
                    ? {
                        ...
                        dailyCalorieGoal: editCalorieGoal ? Number(editCalorieGoal) : undefined,
                    }
                    : u
            ));
            ...
            addToast('Failed to update user: ' + (err instanceof Error ? err.message : String(err)), 'error');
            setActionLoading(false);
        }
    };
        ...
        <>
            ...
            <div className="space-y-6 animate-fade-in">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        ...
                        <p className="text-slate-500 text-sm font-medium mt-1">
                            Manage accounts, roles, and nutritionist assignments directly from Firestore.
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[
                        { label: 'Total Users', value: users.length, color: 'text-slate-800', bg: 'bg-slate-50 border-slate-200' },
                        { label: 'Regular Users', value: users.filter(u => u.role === 'user').length, color: 'text-emerald-700', bg: 'bg-emerald-50 border-emerald-200' },
                        { label: 'Nutritionists', value: users.filter(u => u.role === 'nutritionist').length, color: 'text-blue-700', bg: 'bg-blue-50 border-blue-200' },
                ...
                <Card className="overflow-hidden shadow-xl">
                        ...
                        <table className="w-full text-left text-sm">
                            <thead className="bg-slate-50 border-b border-slate-100 text-slate-500 uppercase font-black text-[10px] tracking-[0.08em]">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 whitespace-nowrap">Assigned Nutritionist</th>
                                    <th className="px-6 py-4 whitespace-nowrap">Goals / Activity</th>
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                <FiUserCheck className={`w-4 h-4 flex-shrink-0 ${user.assignedNutritionistName !== 'Not Assigned' ? 'text-emerald-500' : 'text-slate-300'}`} />
                                                <span className={`text-sm font-medium ${user.assignedNutritionistName !== 'Not Assigned' ? 'text-slate-700' : 'text-slate-400 italic'}`}>
                                            ...
                                            <div className="space-y-1">
                                                <div className="flex items-center gap-1.5">
                                                    ...
                                                    <span className="text-xs font-bold text-slate-700">
                                                        {user.dailyCalorieGoal ? `${user.dailyCalorieGoal} kcal/day` : <span className="text-slate-400 font-normal italic">No goal set</span>}
                                                    </span>
                                                </div>
                                                <div className="flex items-center gap-1.5">
                    ...
                    {loading && (
                        <div className="p-16 text-center">
                            ...
                            <p className="text-slate-400 font-medium animate-pulse text-sm">Loading users from Firestore…</p>
                        </div>
                    )}
                    ...
                    {!loading && error && (
                        <div className="p-12 text-center">
                            ...
                            <button onClick={fetchUsers} className="mt-3 text-xs font-bold text-emerald-600 hover:underline">Retry</button>
                        </div>
                    )}
                    ...
                    {!loading && !error && filtered.length === 0 && (
                        <div className="p-16 text-center">
                            ...
                            <p className="text-slate-400 text-xs mt-1 italic">Try adjusting your search or role filter.</p>
                        </div>
                    )}
                    ...
                    {!loading && filtered.length > PAGE_SIZE && (
                        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between bg-slate-50/50">
                            <p className="text-xs text-slate-500 font-medium">
                                Showing <span className="font-black text-slate-700">{(page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, filtered.length)}</span> of <span className="font-black text-slate-700">{filtered.length}</span> users
                            </p>
                            <div className="flex items-center gap-2">
                                <button
            ...
            {/* view modal */}
            <Modal isOpen={modalType === 'view'} onClose={closeModal} title="User Details" wide>
                {selectedUser && (
                    <div>
                        {/* Avatar + name banner */}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
                            <div>
                                <DetailRow icon={<FiUser className="w-4 h-4" />} label="Name" value={selectedUser.name} />
                                <DetailRow icon={<FiMail className="w-4 h-4" />} label="Email" value={selectedUser.email} />
                                <DetailRow icon={<FiShield className="w-4 h-4" />} label="Role" value={<RoleBadge role={selectedUser.role} />} />
                                <DetailRow icon={<FiActivity className="w-4 h-4" />} label="Age" value={selectedUser.age ? `${selectedUser.age} years` : '—'} />
                            </div>
                            <div>
                                <DetailRow icon={<FiActivity className="w-4 h-4" />} label="Weight" value={selectedUser.weight ? `${selectedUser.weight} kg` : '—'} />
                                <DetailRow icon={<FiActivity className="w-4 h-4" />} label="Height" value={selectedUser.height ? `${selectedUser.height} cm` : '—'} />
                                <DetailRow
                                    icon={<FiUserCheck className="w-4 h-4" />}
                                    label="Assigned Nutritionist"
                                    value={selectedUser.assignedNutritionistName ?? 'Not Assigned'}
                                />
                                <DetailRow
                                    icon={<FiTarget className="w-4 h-4" />}
                                    label="Daily Calorie Goal"
                                    value={selectedUser.dailyCalorieGoal ? `${selectedUser.dailyCalorieGoal} kcal` : '—'}
                                />
                                <DetailRow
                                    icon={<FiTarget className="w-4 h-4" />}
                                    label="Fitness Goal"
           ...
           {/* edit modal */}
            <Modal isOpen={modalType === 'edit'} onClose={closeModal} title="Edit User" wide>
                {selectedUser && (
                        ...
                        <div className="flex justify-end gap-3 pt-2">
                            ...
                            <Button onClick={handleSave} disabled={actionLoading}>
                                {actionLoading ? (
                                    ...
                                    <span className="flex items-center gap-2"><FiCheck className="w-4 h-4" /> Save Changes</span>
                                )}
                            </Button>
                        </div>
                    </div>
            ...
            {/* delete modal */}
            <Modal isOpen={modalType === 'delete'} onClose={closeModal} title="Delete User">
                {selectedUser && (
                    <div className="space-y-5">
                        {/* Warning banner */}
                        <div className="flex items-start gap-4 p-4 bg-red-50 rounded-xl border border-red-200">
                            ...
                            <div>
                                <p className="font-black text-red-700 text-sm">This action cannot be undone</p>
                                <p className="text-xs text-red-500 mt-1">The Firestore document will be permanently deleted.</p>
                            </div>
                        </div>

                        <p className="text-sm text-slate-600">
                            Are you sure you want to delete the account for{' '}
                            <span className="font-black text-slate-800">"{selectedUser.name}"</span>{' '}
                        ...
                        <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-200">
                            ...
                            <div>
                                <p className="font-bold text-slate-800 text-sm">{selectedUser.name}</p>
                                <p className="text-xs text-slate-400">{selectedUser.email} · <RoleBadge role={selectedUser.role} /></p>
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end gap-3">
                            ...
                            <Button variant="danger" onClick={handleDelete} disabled={actionLoading}>
                                {actionLoading ? (
                                    ...
                                    <span className="flex items-center gap-2"><FiTrash2 className="w-4 h-4" /> Yes, Delete</span>
                                )}
                            </Button>
                        </div>
                    </div>
