      ...
      <div className="px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          <Link to="/dashboard" className="flex items-center gap-2 group">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-md group-hover:shadow-emerald-500/30 transition-all">
              <span className="text-white font-black text-xl">N</span>
            </div>
            <span className="text-xl font-black bg-clip-text text-transparent bg-gradient-to-r from-emerald-700 to-teal-700 hidden sm:block">
              NutriTrack
            </span>
          </Link>
            ...
            {user && (
              <>
                <div className="relative" ref={nRef}>
                  <button
                    onClick={() => {
                      if (!notif && unread > 0) markNotificationsAsRead(items);
                      setNotif(!notif);
                      setDrop(false);
                    }}
                    className="p-2 rounded-xl text-slate-500 hover:bg-slate-50 relative transition-all"
                  >
                        ...
                        {items.length === 0 ? (
                          ...
                          <div className="divide-y divide-slate-50">
                            {items.map((n) => (
                              <div
                                key={n.id}
                                className={`px-5 py-4 hover:bg-slate-50 transition-colors ${!n.isRead ? "bg-emerald-50/30" : ""}`}
                              >
                                <div className="flex gap-3">
                                  <div
                                    className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-black flex-shrink-0 ${n.type === "request" ? "bg-orange-400" : "bg-emerald-400"}`}
                                  >
                                    {n.senderName[0]}
                                  </div>
                                  <div>
                    ...
                    <div className="absolute right-0 mt-3 w-48 bg-white rounded-2xl shadow-xl border border-emerald-50 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                      {user.role === "user" && (
                        <Link
                          ...
                          className="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-600 hover:bg-emerald-50 transition-colors"
                        >
