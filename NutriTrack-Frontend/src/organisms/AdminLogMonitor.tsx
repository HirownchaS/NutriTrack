...
const AdminLogMonitor: React.FC = () => {
  ...
  const loadLogs = async (reset = false) => {
    ...
    try {
      const res = await getUserFoodLogs(
        selectedUserId,
        dateRange,
        25,
      ...
      const mapped = res.data.map((item: any) => ({
        ...
        mealType:
          item.mealType || item.meal || item.food?.mealType || "Unknown",
        foodName:
          item.foodName || item.food?.name || item.food?.foodName || "Unknown",
  ...
  const summary = useMemo(() => {
    ...
    const avgCalories = dates.length
      ? Math.round(totalCalories / dates.length)
      : 0;
    ...
    return {
      ...
      totalCarbs,
      totalFats,
      approvedPct: totalCount
        ? Math.round((counts.approved / totalCount) * 100)
        : 0,
      rejectedPct: totalCount
        ? Math.round((counts.rejected / totalCount) * 100)
        : 0,
      pendingPct: totalCount
        ? Math.round((counts.pending / totalCount) * 100)
        : 0,
    };
  }, [logs]);
  ...
  const handleDownloadPdf = () => {
    ...
    y += 18;
    doc.text(
      `Goal: ${selectedUser.fitnessGoal || selectedUser.dailyCalorieGoal || "N/A"}`,
      left,
      y,
    );
    y += 28;
    ...
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-slate-500 text-sm font-medium mt-1">
          Select a user to review their food logs, analytics, and export a PDF
          report.
        </p>
      </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.75fr,1fr]">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                ...
                <p className="text-slate-500 text-sm">
                  Pick a user to load their food logs and summary analytics.
                </p>
              </div>
            </div>
            <select
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-slate-500 text-sm">
                Use a date range to narrow results before exporting.
              </p>
            </div>
      ...
      <Card className="p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-slate-500 text-sm">
              Review the logs for the selected user.
            </p>
          </div>
          <Button
            onClick={handleDownloadPdf}
            disabled={!selectedUser || logs.length === 0}
          ...
          <table className="min-w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {loadingLogs ? (
                ...
                <tr>
                  <td colSpan={8} className="py-12 text-center text-slate-500">
                    No logs found for this user in the selected range.
                  </td>
                </tr>
              ) : (
                logs.map((log) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-[11px] font-bold ${statusStyles[log.status?.toLowerCase() || "pending"]}`}
                      >
