...
import {
  getNutritionists,
  sendNutritionistRequest,
  getUserAllNutritionistRequests,
} from "../firebase/auth";
    ...
    try {
      ...
      try {
        ...
        if (matching.length === 0 && goalValue) {
          ...
          setError(
            "Please complete your profile fitness goal to see personalized matches.",
          );
          matching = normalizedExperts;
        }
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {[1, 2, 3].map((n) => (
            <div
              key={n}
              className="bg-white rounded-2xl h-[400px] animate-pulse border border-slate-100 shadow-sm"
            />
          ))}
        </div>
      ) : !userFitnessGoal ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-emerald-200 shadow-sm transition-all hover:shadow-md">
          ...
          <p className="text-slate-500 mb-8 max-w-md text-center font-medium">
            To provide you with the best experts, we need to know your fitness
            goal. Please update your profile to continue.
          </p>
          <a
        ...
        <>
          {/* Info Box */}
          <div
            className={`${isFallback ? "bg-amber-50 border-amber-100" : "bg-emerald-50 border-emerald-100"} border p-4 rounded-2xl flex items-start gap-3 transition-colors duration-300`}
          >
            <FiInfo
              className={`${isFallback ? "text-amber-600" : "text-emerald-600"} w-5 h-5 mt-0.5 flex-shrink-0`}
            />
            <div className="flex flex-col">
              <p
                className={`text-sm ${isFallback ? "text-amber-700" : "text-emerald-700"} font-bold uppercase tracking-wider text-[10px]`}
              >
                {isFallback
                  ? "Fallback Filter: All Experts"
                  : `Active Filter: ${userFitnessGoal}`}
              </p>
              <p
                className={`text-sm ${isFallback ? "text-amber-700" : "text-emerald-700"} font-medium`}
              >
                {isFallback
          ...
          {filteredNutritionists.length > 0 ? (
            ...
            <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-slate-200">
              ...
              <p className="text-slate-500 mt-1">
                We couldn't find any nutritionists specialized in{" "}
                <strong>{userFitnessGoal}</strong> right now.
              </p>
              <button
                onClick={fetchData}
