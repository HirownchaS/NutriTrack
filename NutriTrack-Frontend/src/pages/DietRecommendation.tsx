...
const DietRecommendation: React.FC = () => {
  ...
  const fetchRecommendation = async () => {
    ...
    try {
      ...
      setError(
        "Failed to load diet recommendations. Please ensure your profile is complete.",
      );
    } finally {
      setIsLoading(false);
    }
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-3 text-slate-500 font-medium">
            Customized for your{" "}
            <span className="text-emerald-600 font-bold">"{data.goal}"</span>{" "}
            goal
          </p>
        </div>
      </header>
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-3 gap-6">
            {[
              ...
              <div
                key={i}
                className={`${macro.bg} backdrop-blur-md p-6 rounded-3xl border ${macro.border} flex flex-col items-center justify-center group hover:scale-105 transition-all duration-300`}
              >
                <span className={`text-3xl font-black ${macro.color}`}>
                  {macro.value}g
                </span>
                <span className="text-black text-[11px] font-black uppercase tracking-[0.2em] mt-2 opacity-80">
                  {macro.label}
                </span>
                <span className="text-slate-500 text-[10px] mt-2 font-bold">
                  {macro.sub}
                </span>
              </div>
            ))}
          ...
          <div className="flex items-start gap-5">
            ...
            <div>
              ...
              <p className="mt-6 text-slate-500 text-sm leading-relaxed decoration-emerald-100 prose">
                This recommendation is generated using a Decision Tree model
                trained on your unique physiological markers. It focuses on
                maintaining metabolic health while steering you toward your{" "}
