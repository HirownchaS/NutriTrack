...
type IntakeHistoryWithGrouped = IntakeHistory & {
...
const HomeSection: React.FC = () => {
  ...
  const isDietPlan = (data: any): data is DietPlan => {
    return (
      ...
      typeof data.nutritionist === "string" &&
    ...
    <div className="space-y-8 animate-fade-in">
      <div>
        ...
        <p className="text-slate-500 mt-1 text-sm font-medium">
          Here's your daily nutritional overview.
        </p>
      </div>
        ...
        <Card className="p-6 bg-linear-to-r from-emerald-600 to-teal-600 text-white relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
              <div>
                ...
                <p className="text-emerald-100 text-sm font-medium mt-1">
                  by {plan.nutritionist} • {plan.daysRemaining} days remaining
                </p>
              </div>
              <div className="flex items-center gap-2 bg-white/15 px-4 py-2 rounded-xl">
                <FiTarget />{" "}
                <span className="text-lg font-black">{plan.dailyCalories}</span>{" "}
                <span className="text-sm text-emerald-200">kcal/day</span>
              </div>
            </div>
            <div className="mt-2">
            ...
            <div className="grid grid-cols-3 gap-3 mt-4">
              {[
                { l: "Protein", v: plan.macros.protein },
                { l: "Carbs", v: plan.macros.carbs },
                { l: "Fats", v: plan.macros.fats },
              ].map((m) => (
                <div
                  key={m.l}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {uploads.map((i) => (
              <Card
                key={i.id}
                className="p-4 hover:-translate-y-1 transition-all group"
              >
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="text-slate-400 font-medium">
                    P:{i.protein}g C:{i.carbs}g F:{i.fats}g
                  </span>
                </div>
              </Card>
            ))}
...
const FoodUploadSection: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <DailyGoalTracker />
      <UploadSection
        onDetectionComplete={(r) => setResults((p) => [...p, r])}
      />
      {results.length > 0 && (
  ...
  return (
    <div className="space-y-8 animate-fade-in">
        ...
        <Card className="p-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            ...
            {(() => {
              ...
              return (
                <div
                  className={`flex items-center gap-2 px-4 py-2 rounded-xl border font-bold text-sm ${s.c}`}
                >
                  <s.i />
                  {s.l}
                </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            {[
              {
                l: "Protein",
                v: history.macros?.protein,
                c: "text-emerald-600",
                b: "bg-emerald-50",
              },
              {
                l: "Carbs",
                v: history.macros?.carbs,
                c: "text-blue-600",
                b: "bg-blue-50",
              },
              {
                l: "Fats",
                v: history.macros?.fats,
                c: "text-orange-600",
                b: "bg-orange-50",
              },
            ].map((m) => (
              <div key={m.l} className={`${m.b} p-3 rounded-2xl text-center`}>
                <p className={`text-lg font-black ${m.c}`}>{m.v || 0}g</p>
          ...
          <div className="h-3 bg-slate-100 rounded-full overflow-hidden">
            <div
              className={`h-full transition-all duration-700 ${history.compliance === "over" ? "bg-red-500" : history.compliance === "under" ? "bg-orange-400" : "bg-emerald-500"}`}
              style={{
                width: `${Math.min((history.consumed / history.dailyGoal) * 100, 100)}%`,
              }}
            />
          </div>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <ManualFoodLogForm onSuccess={fetch} />
        <GoalForm onSuccess={fetch} />
      </div>
              ...
              <div className="space-y-3">
                {history.grouped && history.grouped[mealType]?.length > 0 ? (
                  history.grouped[mealType].map((i: any) => (
                    <div
                      key={i.id}
                      className="flex items-center gap-4 p-4 rounded-xl bg-slate-50 hover:bg-slate-100 transition-colors border border-slate-100"
                    >
                  ...
                  <p className="text-xs text-slate-400 font-medium italic p-2">
                    No items logged for {mealType} yet.
                  </p>
                )}
              </div>
            </Card>
...
const Dashboard: React.FC = () => {
  ...
  const error =
    new URLSearchParams(loc.search).get("error") === "access-denied";
          ...
          element={
            <div className="space-y-8 animate-fade-in">
              <div>
                ...
                <p className="text-slate-500 mt-1 text-sm font-medium">
                  Chat with your assigned nutritionist.
                </p>
              </div>
              <ChatModule />
            </div>
