...
export const getAdminStats = async () => {
  ...
  return {
    data: {
      ...
      totalNutritionists: users.docs.filter(
        (d) => d.data().role === "nutritionist",
      ).length,
      totalFoodLogs: logs.size,
      activeDietPlans: plans.size,
      pendingNutritionistRequests: requests.size,
      systemWideAvgCalories:
        logs.size > 0 ? Math.round(totalCals / logs.size) : 0,
    },
  };
};
...
export const manageNutritionist = async (action: string, userId: string) => {
  await updateDoc(doc(db, "users", userId), {
    role: action === "add" ? "nutritionist" : "user",
...
export const deleteNutritionist = async (id: string) => {
  ...
  const users = await getDocs(
    query(collection(db, "users"), where("nutritionistId", "==", id)),
  );
  users.forEach((d) => batch.update(d.ref, { nutritionistId: null }));
  const altUsers = await getDocs(
    query(collection(db, "users"), where("assignedNutritionistId", "==", id)),
  );
  altUsers.forEach((d) =>
    batch.update(d.ref, { assignedNutritionistId: null }),
  );
...
export const getAdminFoodItems = async () => {
  ...
  return {
    data: (res.data || []).map((i: any) => ({ ...i, protein: i.proteins })),
  };
};

export const createFoodItem = async (data: any) =>
  api.post("/admin/food", { ...data, proteins: data.protein });
export const updateFoodItem = async (id: string, data: any) =>
  api.put(`/admin/food/${id}`, { ...data, proteins: data.protein });
export const deleteFoodItem = async (id: string) =>
  api.delete(`/admin/food/${id}`);
