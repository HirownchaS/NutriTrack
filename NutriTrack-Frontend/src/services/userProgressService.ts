...
export const computeCalorieCompliance = (
  ...
  return {
    ...
    level,
    label,
    dailyAvgCalories: Math.round(avgDaily),
    dailyGoal,
  };
};
  ...
  if (totalMacroGrams > 0 && profile.proteinGoal > 0) {
    ...
    const avgDeviation =
      (Math.abs(1 - proteinRatio) +
        ...
        Math.abs(1 - fatsRatio)) /
      3;
    macroScore = Math.max(
      0,
      Math.min(100, Math.round((1 - avgDeviation) * 100)),
    );
  }
...
export const getUserProgressData = async (
  ...
  // Compute metrics
  const compliance = computeCalorieCompliance(
    trendLogs,
