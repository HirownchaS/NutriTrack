  ...
  const assigned = assignedSnap.docs
    ...
    .map((userDoc) => {
      ...
      return {
        ...
        fitnessGoal: u.fitnessGoal || "Wellness",
        status: "accepted",
        date: u.createdAt
...
export const getUserFullData = async (userId: string) => {
  ...
  return {
    data: {
      id: userId,
      name: u.name || "User",
      weightProgress: u.weight ? [u.weight] : [],
      recentLogs: recent,
      dietPlan: {
...
export const approveFoodLog = async (
  logId: string,
...
export const getNutritionistDashboard = async () => {
  ...
  for (const id of uids) {
    if ((await getDoc(doc(db, "diet_recommendations", id))).exists())
      activePlans++;
  }
    ...
    try {
      ...
      for (const l of logs.docs) {
        ...
        activity.push({
          id: l.id,
          type: "food_log",
          user: uDoc.data()?.name || "User",
          action: `logged ${ld.foodName || "a meal"} (${ld.calories || 0} kcal)`,
          time: ld.createdAt
            ? getRelativeTime(ld.createdAt.toDate())
            : "recently",
          icon: "📸",
        });
      }
