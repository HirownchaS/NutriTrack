...
const NutritionChart: React.FC<NutritionChartProps> = ({ nutrition }) => {
  ...
  const dietRecommendation = (() => {
    ...
    if (totalCalories > 2500)
      return {
        type: "warning",
        msg: "High calorie meal detected. Consider lighter options next time.",
      };
    if (totalProtein < 10 && totalCalories > 300)
      return {
        type: "info",
        msg: "Low protein intake. Adding lean protein can boost satiety.",
      };
    if (totalCarbs > 150)
      return {
        type: "info",
        msg: "High carb content. Balance with fiber-rich vegetables.",
      };
    if (totalFiber > 10)
      return {
        type: "success",
        msg: "Great fiber intake! This supports digestion and gut health.",
      };
    if (totalCalories < 100)
      return {
        type: "info",
        msg: "Light meal detected. Ensure you meet your daily calorie needs.",
      };
    return null;
  })();
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-6">
        ...
        <div className="relative flex-shrink-0">
          <svg
            ...
            aria-label="Macro distribution chart"
          >
            {/* Background circle */}
            ...
            {total > 0 ? (
              arcs.map((d, i) => (
                ...
                >
                  <title>
                    {MACROS[i].label}: {values[i].toFixed(1)}g
                  </title>
                </path>
              ))
            ) : (
      ...
      {dietRecommendation && (
        <div
          className={`flex items-start gap-3 p-3 rounded-xl text-sm border-l-4 ${
