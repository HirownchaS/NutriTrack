...
const calculateNutrition = (foodName: string, portionType: string) => {
  ...
  let base = Object.entries(NUTRITION_BASE).find(([k, _]) =>
    key.includes(k),
  )?.[1];
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-100">
                    <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-wider">
                      Food Name
                    </th>
                    <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-wider text-center">
                      Portion
                    </th>
                    <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-wider text-center">
                      Calories
                    ...
                    <th className="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-wider text-right">
                      Meal
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  <tr className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-4 py-4 text-sm font-bold text-slate-700">
                      {lastSavedLog.foodName}
                    </td>
                    <td className="px-4 py-4 text-sm font-bold text-slate-500 text-center">
                      {lastSavedLog.portion}
                    </td>
                    <td className="px-4 py-4 text-sm font-black text-emerald-600 text-center">
                      {lastSavedLog.calories} kcal
                    </td>
                    <td className="px-4 py-4 text-sm font-bold text-slate-600 text-center">
                      {lastSavedLog.protein}g
                    </td>
                    <td className="px-4 py-4 text-sm font-bold text-slate-600 text-center">
                      {lastSavedLog.carbs}g
                    </td>
                    <td className="px-4 py-4 text-sm font-bold text-slate-600 text-center">
                      {lastSavedLog.fats}g
                    </td>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              onChange={(e) =>
                setFormData({ ...formData, portionType: e.target.value })
              }
            >
