...
const ChatModule: React.FC<ChatModuleProps> = ({ recipientId, recipientName }) => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = subscribeToMessages(user.uid, receiverId, (msgs: Message[]) => {
            setMessages(msgs);
        });
    ...
    return (
        <Card className="flex flex-col h-[500px] p-0 overflow-hidden border-emerald-100">
            ...
            <div className="bg-emerald-500 p-4 text-white flex items-center gap-3">
                ...
                <div>
                    <h3 className="font-bold">Chat with {actualRecipientName|| 'Nutritionist'}</h3>
                    <p className="text-xs text-emerald-100">Online • Get professional advice</p>
                </div>
            </div>
            ...
            >
                {!actualRecipientId ? (
                    <div className="text-center py-20 px-10">
                        ...
                        <p className="text-slate-400 text-sm mt-1 italic">Please select a nutritionist from your dashboard to start a conversation.</p>
                    </div>
                ) : messages.length === 0 ? (
                    ...
                    messages.map((msg) => (
                        ...
                        >
                            <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 shadow-sm text-sm ${msg.sender_id === user?.uid
                                ? 'bg-emerald-600 text-white rounded-tr-none'
                                : 'bg-white text-slate-700 border border-slate-100 rounded-tl-none'
                                }`}>
                                {msg.content}
                                <p className={`text-[10px] mt-1 opacity-70 ${msg.sender_id === user?.uid ? 'text-right' : 'text-left'}`}>
